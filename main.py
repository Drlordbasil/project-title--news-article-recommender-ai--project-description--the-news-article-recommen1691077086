...
class TextPreprocessor:
    ...
    def preprocess(self, text):
        ...
        filtered_tokens = [self.stemmer.stem(token) for token in tokens if token not in self.stop_words]
    ...
    def get_recommendations(self, user_id):
        ...
        for article_id in range(len(self.articles)):
            if article_id not in articles_read:
                score = sum([self.similarity_matrix[article_id][read_id] for read_id in articles_read])
                article_scores[article_id] = score
        
        sorted_articles = sorted(article_scores.items(), key=lambda x: x[1], reverse=True)
        recommended_article_ids = [article_id for article_id, _ in sorted_articles]
...
class NotificationManager:
    ...
    def get_user_notifications(self, user_id):
        user_notifications = [notification for notification in self.notifications if notification['user_id'] == user_id]
...
class ArticleSaver:
    ...
    def get_user_saved_articles(self, user_id):
        user_saved_articles = [saved_article for saved_article in self.saved_articles if saved_article['user_id'] == user_id]
...
def news_article_recommender():
    ...
    preprocessed_corpus = [preprocessor.preprocess(article['summary']) for article in articles]
    ...
    recommendation_engine = RecommendationEngine(users, articles, similarity_matrix)
